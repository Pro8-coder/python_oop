...
дереву) для вектора x из корневого узла дерева root (возвращает значение
узла - атрибут value).
...
class TreeObj:
    """Класс, представляет вершины и листья решающего дерева."""

    def __init__(self, indx: int, value: str = None) -> None:
        """
        Инициализация объекта TreeObj.

        :param indx: Индекс, который проверяется в вершине дерева.
        :param value: Значение, хранящееся в вершине (None у не листьев).
        """
        self.indx = indx
        self.value = value
        self.__left = None
    ...
    def left(self) -> 'TreeObj':
        """
        Возвращает левого потомка вершины.

        :return: Левый потомок вершины.
        """
        return self.__left
    ...
    def left(self, left: 'TreeObj') -> None:
        """
        Устанавливает левого потомка вершины.

        :param left: Левый потомок вершины.
        """
        self.__left = left
    ...
    def right(self) -> 'TreeObj':
        """
        Возвращает правого потомка вершины.

        :return: Правый потомок вершины.
        """
        return self.__right
    ...
    def right(self, right: 'TreeObj') -> None:
        """
        Устанавливает правого потомка вершины.

        :param right: Правый потомок вершины.
        """
        self.__right = right


class DecisionTree:
    """Класс для работы с решающим деревом."""

    @classmethod
    def predict(cls, root: TreeObj, x: list[int]) -> str | None:
        """
        Построение прогноза (прохода по решающему дереву) для вектора x.

        :param root: Корневой узел дерева.
        :param x: Вектор с бинарными значениями (0 или 1).
        :return: Значение листа дерева (строка) или None, если дерево пустое.
        """
        if root is None:
            return None
    ...
    def add_obj(cls, obj: TreeObj, node: TreeObj = None, left: bool = True) \
            -> TreeObj:
        """
        Добавление вершины в решающее дерево.

        :param obj: Добавляемый объект (вершина дерева).
        :param node: Родительский узел, к которому присоединяется obj.
        :param left: Определяет ветвь дерева (True - левая, False - правая).
        :return: Добавленный объект (вершина дерева).
        """
        if node is None:
            return obj
        elif left:
