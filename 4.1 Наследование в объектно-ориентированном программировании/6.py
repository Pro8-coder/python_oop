...
activation - функция активации (строка, например: 'linear', 'relu',
'sigmoid'). И в каждом объекте класса Dense также должен автоматически
...
объект (слой) нейронной сети. Объект этого класса является итератором, который
в цикле for последовательно возвращает объекты (слои) нейронной сети.
...
class Layer:
    ...
    Обеспечивает функционал для связи между последующими слоями через атрибут
    next_layer.

    :ivar next_layer: Ссылка на следующий слой нейронной сети
    :ivar name: Название типа слоя
    """
    ...
    def __call__(self, layer: 'Layer') -> 'Layer':
        """
        Связывает текущий слой со следующим слоем.

        :param layer: Следующий слой для связи
        :return: Связанный следующий слой
        :raises ValueError: Если переданный объект не является Layer
        """
...
class Input(Layer):
    """
    Класс входного слоя нейронной сети.

    :ivar inputs: Количество входных параметров
    """
...
class Dense(Layer):
    ...
    :ivar inputs: Количество входных параметров
    :ivar outputs: Количество выходных параметров
    :ivar activation: Функция активации
    """
...
class NetworkIterator:
    """
    Итератор для последовательного перебора слоев нейронной сети.

    :ivar network: Начальный слой для итерации
    """
    ...
    def __iter__(self) -> Iterator['Layer']:
        """
        Итератор для последовательного прохода по всем слоям нейронной сети.

        Генератор, который последовательно возвращает все слои нейронной сети,
        начиная от текущего слоя (network) до последнего связанного слоя,
        использует yield.
        """
